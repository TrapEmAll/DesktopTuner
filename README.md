...
- Opens a taskbar overlay on the primary display or every connected display, with live top-level window switching, minimize commands, a clock, and a shortcut to the companion Start menu on that display. App buttons can be centered while Start stays left, matching the selected Windows taskbar alignment. Edge, size, full-edge, floating, or segmented layout, display coverage, and auto-hide preferences persist between app launches.
