...
The taskbar overlays cover the native taskbars visually but leave them running underneath; they currently have no notification-area icons, and drag-and-drop accepts `.exe` files only. Appearance follows each display's effective DPI, but mixed-DPI and hot-plug behavior still needs manual validation. Some Windows registry preferences and the classic context-menu switch may vary across Windows builds. Start's recent-items option is a Windows privacy setting shared with Jump Lists and File Explorer. Replacing the Windows key menu and reproducing legacy taskbar/Explorer styling remain parity work.
