...
- Opens a companion File Explorer with multi-tab browsing, per-tab navigation and search history, Ctrl+T/Ctrl+W/Ctrl+Tab shortcuts, and an option to open folders in a new tab. It also has a classic command strip, quick access to common folders and drives, recursive name search through the current folder tree, clickable Name/Date/Type/Size column sorting, and a bottom details pane that can be hidden or resized. Select multiple files or folders and cut, copy, and paste them with the toolbar, context menu, or Ctrl+X/C/V; cut items are dimmed until moved. It follows the app’s Windows preferences for startup location, hidden items, and file extensions. Create folders, rename items, send deletions to the Recycle Bin, and refresh from the toolbar or context menu; F2, Delete, F5, and Ctrl+Shift+N are supported. Double-click folders to browse and files to launch with their default app.
