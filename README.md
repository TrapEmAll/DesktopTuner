...
- Opens a companion File Explorer with a classic command strip, quick access to common folders and drives, recursive name search through the current folder tree, and a bottom details pane. Create folders, rename items, send deletions to the Recycle Bin, and refresh from the toolbar or context menu; F2, Delete, F5, and Ctrl+Shift+N are supported. Double-click folders to browse and files to launch with their default app.
