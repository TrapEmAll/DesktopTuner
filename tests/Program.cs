...
Check(9, StartMenuPlaceCatalog.DropdownPlaces.Count, "show the supported Start places with dropdown navigation");
...
Check(20, defaultStartPlaces.Order!.Count, "include every supported system place in the default order");
Check(20, defaultStartPlaces.Visible!.Count, "show every system place by default");
...
Check("run,documents,downloads,music,pictures,videos,libraries,favorites,devices-printers,network,user-profile,computer,recycle-bin,control-panel,default-programs,network-connections,troubleshooting,windows-tools,help-support,recent", string.Join(',', customStartPlaces.Order!), "retain a valid custom place order and append missing choices");
...
preferencesStore.Save(expectedPreferences);
    ...
    Check(20, preferencesStore.Load().StartMenuPlaces!.Visible!.Count, "show all Start places for older preference files");
