...
On Windows 11 Pro, Taskbar settings now offer **Use Desktop Tuner at sign-in** and **Restore Windows Explorer** for this per-user policy. Setup writes the installed executable path only when no other custom shell is configured; restore removes that value only while it still points to Desktop Tuner. The same policy can be configured manually in Local Group Policy Editor under **User Configuration > Administrative Templates > System > Custom User Interface**, with **Interface file name** set to the full path of `DesktopTuner.exe` (without command-line arguments). At sign-in, Desktop Tuner detects that the current-user policy targets its own executable and starts in `--shell-host` mode. Changes take effect at the next sign-in.

Before testing, keep a separate administrator account available and save work. To recover the current session, use Ctrl+Alt+Delete, open Task Manager, choose **Run new task**, and start `explorer.exe`. Then run `gpedit.msc`, set **Custom User Interface** to **Not Configured** or **Disabled**, and sign out and back in. This policy path and the app's detection are covered by source review and automated tests, but actual logon, crash, and recovery behavior still requires validation on a Windows 11 Pro desktop. Do not use it as a daily shell until that validation is complete.
