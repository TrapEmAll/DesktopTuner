...
On Windows 11 Pro, open Local Group Policy Editor and enable **User Configuration > Administrative Templates > System > Custom User Interface**. Set **Interface file name** to the full path of the installed `DesktopTuner.exe` (without command-line arguments). At sign-in, Desktop Tuner detects that the current-user policy targets its own executable and starts in `--shell-host` mode. The app does not write or change this policy; disabling or clearing it remains under the user's control. Policy changes take effect at the next sign-in.
