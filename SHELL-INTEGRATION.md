...
On Windows 11 Pro, Taskbar settings now offer **Use Desktop Tuner at sign-in** and **Restore Windows Explorer** for this per-user policy. Setup writes the installed executable path only when no other custom shell is configured; restore removes that value only while it still points to Desktop Tuner. The same policy can be configured manually in Local Group Policy Editor under **User Configuration > Administrative Templates > System > Custom User Interface**, with **Interface file name** set to the full path of `DesktopTuner.exe` (without command-line arguments). At sign-in, Desktop Tuner detects that the current-user policy targets its own executable and starts in `--shell-host` mode. A supervisor retries a failed shell process once and then starts Explorer; a normal shell exit also starts Explorer for the current session. Changes take effect at the next sign-in.
