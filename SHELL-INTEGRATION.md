...
An experimental desktop host can be started manually with `DesktopTuner.exe --desktop-host`. It displays the configured wallpaper and visible items from the user and public Desktop folders on the primary display, opens items through the Windows shell, and supports refresh and folder creation. It is a prototype that runs alongside Explorer: native desktop icons may remain behind it, it has no multi-monitor layout or shell namespace objects such as This PC and Recycle Bin, and it has no automatic sign-in or Shell Launcher configuration.
