...
An experimental desktop host can be started manually with `DesktopTuner.exe --desktop-host`. It displays the configured wallpaper and visible items from the user and public Desktop folders on the primary display, adds This PC and Recycle Bin as shell namespace entries with native shell icons, opens items through the Windows shell, watches for Desktop content changes, supports dragging filesystem items out to other apps, and provides refresh and folder creation. It is a prototype that runs alongside Explorer: native desktop icons may remain behind it, it has no multi-monitor layout or full shell namespace support, and it has no automatic sign-in or Shell Launcher configuration.
