...
public sealed class WindowsKeyGesture
{
        ...
        Func<int, bool>? canActivateLastActivePinnedApp = null)
    {
        ...
        if (_heldWindowsKey is not null && !_forwarded)
        {
            ...
            if (key is (uint)'A' or (uint)'F' or (uint)'G' or (uint)'H' or (uint)'I' or (uint)'K' or (uint)'L' or (uint)'N' or (uint)'P' or (uint)'Q' or (uint)'S' or (uint)'U' or (uint)'V' or (uint)'W' or (uint)'Z' or 0x09 or 0x13 or 0x20 && !controlPressed && !altPressed && !shiftPressed &&
            ...
            if (key == (uint)'C' && !controlPressed && !altPressed && !shiftPressed && canOpenCopilot?.Invoke() == true)
            {
                _taskbarShortcutConsumed = true;
                _suppressedShortcutKeys.Add(key);
            ...
            if (key is 0xBA or 0xBE && !controlPressed && !altPressed && !shiftPressed && canOpenEmojiPanel?.Invoke() == true)
            {
                _taskbarShortcutConsumed = true;
                _suppressedShortcutKeys.Add(key);
                return WindowsKeyAction.OpenEmojiPanel;
            }
            if (key == (uint)'J' && !controlPressed && !altPressed && !shiftPressed && canOpenWindowsTip?.Invoke() == true)
            {
                _taskbarShortcutConsumed = true;
                _suppressedShortcutKeys.Add(key);
                return WindowsKeyAction.OpenWindowsTip;
            }
