...
public enum TaskbarSearchStyle { None, Button, Box }
...
public sealed class DesktopPreferencesStore
{
    ...
    public DesktopPreferences Load()
    {
        ...
        try
        {
            ...
            if (value is null || !Enum.IsDefined(value.TaskbarEdge) || !Enum.IsDefined(value.TaskbarSize) || !Enum.IsDefined(value.StartMenuStyle) || !Enum.IsDefined(value.TaskbarLayout) || !Enum.IsDefined(value.TaskbarGrouping) || !Enum.IsDefined(value.TaskbarButtonAlignment) || !Enum.IsDefined(value.TaskbarIconSize) || !Enum.IsDefined(value.TaskbarButtonSpacing) || !Enum.IsDefined(value.TaskbarButtonEffect) || !Enum.IsDefined(value.TaskbarWindowDisplayMode) || !Enum.IsDefined(value.TaskbarVisualStyle) || !Enum.IsDefined(value.TaskbarLabelVisibility) || !Enum.IsDefined(value.TaskbarSearchStyle))
