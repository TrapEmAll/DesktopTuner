...
| Taskbar | Tray, taskbar context menus, widgets, flyouts, and multi-monitor behavior | Partial: per-window and grouped minimize/close menus, a tray control that focuses Windows' native notification area, date/time clock opening the native notifications/calendar flyout, and overlays on all displays; directly rendering native app/system tray icons, widgets, and other shell flyouts remain |
